...
if not os.path.exists(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}"):
    os.mkdir(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}")
...
hd_data.write_uvh5(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.uvh5", clobber=True)
del data, flag, nsample, hd_data
redcal.redcal_run(input_data=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.uvh5", clobber=True, solar_horizon=90, verbose=True)
abscal.post_redcal_abscal_run(data_file=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.uvh5", redcal_file=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.omni.calfits", model_files=[model_file], clobber=True, data_solar_horizon=90, model_solar_horizon=90)

cs=smooth_cal.CalibrationSmoother(calfits_list=sorted(glob.glob(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_*.abs.calfits")))
cs.time_freq_2D_filter(time_scale=21600)
cs.write_smoothed_cal(clobber=True, output_replace=(".abs.",".smooth_abs."))


apply_cal.apply_cal(data_infilename=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.uvh5", data_outfilename=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}_smoothcal.uvh5", new_calibration=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.smooth_abs.calfits", clobber=True)

vc = vis_clean.VisClean(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}_smoothcal.uvh5")
vc.read()
...
vc.write_filtered_data(filled_outfilename=f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}_filtered.uvh5", clobber=True)
os.remove(f"/users/kshahin/kshahin/HERA_Calibration/Dayfx_{day}/data_{day}_{chunk}.uvh5")
